...
const CommitmentsScreen = () => {
  ...
  const selectedTypeName = (typeFilterOptions.find(t => t.id === selectedTypeFilter)?.name) ?? 'الكل';
  ...
  const renderCommitmentCard = (commitment: any) => {
    const isRecurring = commitment?.isRecurring === true || commitment?.type === 'rent' || (!commitment?.totalAmount && (commitment?.progress === null || commitment?.progress === undefined));
    const totalAmount = !isRecurring && (typeof commitment.progress === 'number' && commitment.progress > 0 && commitment.progress < 1)
      ? commitment.remainingAmount / (1 - commitment.progress)
      : (!isRecurring ? (commitment.totalAmount ?? null) : null);
    return (
    <TouchableOpacity
      key={commitment.id}
    ...
    >
      <View style={styles.cardContent}>
        ...
        <View style={styles.amountSection}>
          {isRecurring ? (
            <View style={[styles.amountRow, { justifyContent: 'flex-end' }]}>
              <View style={styles.amountMetric}>
                <Text style={styles.monthlyLargeAmount}>
                  {commitment.monthlyPayment.toLocaleString()} د.ك/شهر
                </Text>
                <Text style={styles.amountLabel}>التزام شهري مستمر</Text>
              </View>
            </View>
          ) : (
            <>
              <View style={styles.amountRow}>
                ...
                {totalAmount ? (
                  <View style={[styles.amountMetric, { marginLeft: 16 }]}> 
                    <Text style={styles.remainingAmount}>
                      {Math.round(totalAmount * 1000) / 1000} د.ك
                    </Text>
                    <Text style={styles.amountLabel}>أصل المبلغ</Text>
                  </View>
                ) : null}
              </View>
    ...
    <View style={styles.commitmentsSection}>
      <View style={styles.sectionHeader}>
        <Text style={styles.sectionTitle}>
          {selectedTypeFilter === 'all' ? 'الالتزامات المالية' : `الالتزامات • ${selectedTypeName}`}
        </Text>
      ...
      {filteredCommitments.length === 0 && (
        <View style={styles.emptyState}>
          ...
          <Text style={styles.emptySubtitle}>
            عدّل الفلاتر أو جرّب كلمات بحث مختلفة.
          </Text>
          <TouchableOpacity
  ...
  // ملخص النوع المختار: إجمالي القسط الشهري وإجمالي المتبقي
  const typeMonthlyTotal = React.useMemo(() => (
    selectedTypeFilter === 'all' ? 0 : typeFilteredCommitments.reduce((sum, c) => sum + (c.monthlyPayment || 0), 0)
  ), [selectedTypeFilter, typeFilteredCommitments]);

  const typeRemainingTotal = React.useMemo(() => (
    selectedTypeFilter === 'all' ? 0 : typeFilteredCommitments.reduce((sum, c) => sum + (c.remainingAmount || 0), 0)
  ), [selectedTypeFilter, typeFilteredCommitments]);

  const renderTypeSummary = () => {
    if (selectedTypeFilter === 'all') return null;
    return (
        ...
        <View style={styles.typeSummaryRow}>
          <View style={[styles.typeSummaryCard]}> 
            ...
            <View style={styles.typeSummaryTextBox}>
              ...
              <Text style={styles.typeSummaryValue}>{typeMonthlyTotal.toLocaleString()} د.ك</Text>
            </View>
          </View>
          <View style={[styles.typeSummaryCard]}> 
            ...
            <View style={styles.typeSummaryTextBox}>
              ...
              <Text style={styles.typeSummaryValue}>{typeRemainingTotal.toLocaleString()} د.ك</Text>
            </View>
          </View>
        </View>
      </View>

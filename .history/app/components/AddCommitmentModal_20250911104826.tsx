...
const AddCommitmentModal: React.FC<AddCommitmentModalProps> = ({
  ...
  const [selectionLevel, setSelectionLevel] = useState(1); // 1 for category, 2 for type
  const [selectedCategory, setSelectedCategory] = useState<CommitmentCategory | null>(null);
  ...
  const [calculatedTotalRepayment, setCalculatedTotalRepayment] = useState<number>(0);
  ...
  const commitmentCategories: CommitmentCategory[] = [
    {
      id: 'loans_financing',
      name: 'قروض وتمويل',
      icon: 'cash',
      ...
      types: [
        ...
        {
          id: 'installments_with_interest',
          name: 'أقساط بفائدة',
          icon: 'analytics',
          color: '#D946EF',
          description: 'أقساط شركات التمويل مع فائدة'
        },
        {
          id: 'credit_card',
          name: 'بطاقة ائتمان',
    ...
    },
    {
      ...
      types: [
        ...
        },
        {
          ...
          icon: 'people-circle',
          color: '#22C55E',
          description: 'الجمعيات المالية بين الأفراد'
        },
      ]
    },
    {
      id: 'installments_bills',
      name: 'أقساط وفواتير',
      icon: 'receipt',
      color: '#3B82F6',
      types: [
  ...
  const financing_companies: Institution[] = [
    { id: 'almulla', name: 'الغانم للتمويل', type: 'financing', icon: 'business', color: '#D946EF' },
    { id: 'kfh_finance', name: 'بيت التمويل الكويتي للتمويل', type: 'financing', icon: 'business', color: '#16A34A' },
    { id: 'nbk_finance', name: 'بنك الكويت الوطني للتمويل', type: 'financing', icon: 'business', color: '#1E40AF' },
    { id: 'qnb_finance', name: 'QNB للتمويل', type: 'financing', icon: 'business', color: '#8B5CF6' },
  ];
  ...
  const renderTypeSelector = () => {
    const chunkedTypes = commitmentTypes.reduce((acc, item, index) => {
  ...
  const renderInstitutionSelector = () => {
    ...
    switch (selectedType?.id) {
      ...
      case 'installments':
        ...
        break;
      case 'installments_with_interest':
        institutions = financing_companies;
        sectionTitle = 'اختر شركة التمويل';
        break;
      case 'bnpl':
  ...
  const renderPersonalContactSelector = () => {
    // TODO: Implement personal contacts management. This is a temporary solution.
    return (
      <View style={styles.stepContent}>
        <Text style={styles.stepTitle}>إضافة دين شخصي</Text>
        <Text style={styles.stepDescription}>أدخل اسم الشخص وتفاصيل الدين</Text>
        
        <View style={styles.inputGroup}>
          <Text style={styles.inputLabel}>اسم الشخص *</Text>
          <TextInput
            style={styles.textInput}
            placeholder="مثال: محمد الأحمد"
            value={formData.personName || ''}
            onChangeText={(value) => updateFormData('personName', value)}
            textAlign="right"
          />
        </View>
      </View>
  ...
  const renderDetailsForm = () => {
    const updateFormData = (key: string, value: any) => {
      ...
      // Auto-calculate loan payment for bank loans and installments with interest
      if ((selectedType?.id === 'bank_loan' && 
          formData.principal && formData.interestRate && formData.termMonths) ||
          (selectedType?.id === 'installments_with_interest' &&
          formData.totalAmount && formData.interestRate && formData.installmentCount)) {
        const principal = selectedType?.id === 'bank_loan' ? formData.principal : formData.totalAmount;
        const months = selectedType?.id === 'bank_loan' ? formData.termMonths : formData.installmentCount;

        const { payment, totalInterest, totalRepayment } = calculateLoanPayment(
        ...
        setCalculatedTotalRepayment(totalRepayment);
      }
    };
        ...
        <ScrollView style={styles.formContainer} showsVerticalScrollIndicator={false}>
          ...
          {selectedType?.id === 'bank_loan' && (
            <>
              ...
              <View style={styles.inputGroup}>
                <Text style={styles.inputLabel}>تاريخ بداية القرض</Text>
                <TextInput
                  style={styles.textInput}
              ...
              <View style={styles.inputGroup}>
                <Text style={styles.inputLabel}>تاريخ نهاية القرض</Text>
                <TextInput
                  style={styles.textInput}
                  placeholder="dd/mm/yyyy"
              ...
              {calculatedPayment > 0 && (
                <View style={styles.calculatorResult}>
                  ...
                  <Text style={styles.calculatorLabel}>إجمالي الفوائد:</Text>
                  <Text style={styles.calculatorAmount}>{calculatedTotalInterest.toFixed(3)} د.ك</Text>
                  <Text style={styles.calculatorLabel}>المبلغ الإجمالي بعد الفوائد:</Text>
                  <Text style={styles.calculatorAmount}>{calculatedTotalRepayment.toFixed(3)} د.ك</Text>
                </View>
              )}
            </>
          )}
          ...
          {/* Installments with Interest Fields */}
          {selectedType?.id === 'installments_with_interest' && (
            <>
              <View style={styles.inputGroup}>
                <Text style={styles.inputLabel}>إجمالي المبلغ (د.ك) *</Text>
            ...
            <>
              ...
              <View style={styles.inputGroup}>
                ...
                <TextInput
                  ...
                  onChangeText={(value) => updateFormData('dueDay', value)}
                  keyboardType="number-pad"
                  textAlign="right"
                />
              </View>
          ...
          {selectedType?.id === 'cooperatives' && (
            <>
              ...
              <View style={styles.inputGroup}>
                <Text style={styles.inputLabel}>مدة الالتزام (عدد الدفعات) *</Text>
                <TextInput
                  style={styles.textInput}
                  placeholder="10"
                  value={formData.installmentsCount || ''}
                  onChangeText={(value) => updateFormData('installmentsCount', value)}
                  keyboardType="number-pad"
                  textAlign="right"
                />
              </View>
